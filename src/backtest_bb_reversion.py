...
실행:
  python -m src.backtest_bb_reversion           # 기본 유니버스
  python -m src.backtest_bb_reversion AAPL
"""
...
from . import strategy_bb_reversion as bb
...
class BacktestState:
    cash: float
    position: bb.Position | None = None
...
def enter(state: BacktestState, qty: int, price: float, date: pd.Timestamp) -> None:
    ...
    if cost > state.cash:
        ...
        cost = qty * price * (1 + COMMISSION / 2)
    state.cash -= cost
    state.position = bb.Position(entry_date=date, entry_price=price, qty=qty)
...
def run_backtest(df: pd.DataFrame, initial_capital: float = INITIAL_CAPITAL) -> BacktestState:
    ...
    for i, date in enumerate(dates):
        ...
        if state.position is not None:
            ex = bb.check_exit(row, state.position, date)
            if ex.should_exit:
        ...
        if state.position is None:
            if state.last_exit_date is not None:
                ...
                if days_since < bb.REENTRY_COOLDOWN_DAYS:
                    state.equity_curve[date] = equity(state, price)
                    continue
            if i >= 1:
                prev_row = df.iloc[i - 1]
                sig = bb.check_entry(prev_row)
                if sig.valid:
...
def print_summary(symbol: str, state: BacktestState, df: pd.DataFrame, initial: float) -> None:
    ...
    if trades:
        ...
        print(f"  체제 분포: BULL {by_regime[mr.REGIME_BULL]}, "
